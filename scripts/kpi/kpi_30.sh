...
if test $p -gt 0; then
put_lt1s_size=$(awk '$12<1000' $put_log | awk '{sum+=$13}END{if (NR > 0) {printf("%.2f", sum/NR/1024)}else{print "-"}}')
put_gt1s_size=$(awk '$12>=1000 && $12<=10000' $put_log | awk '{sum+=$13}END{if (NR > 0) {printf("%.2f", sum/NR/1024)}else{print "-"}}')
put_gt10s_size=$(awk '$12>10000' $put_log | awk '{sum+=$13}END{if (NR > 0) {printf("%.2f", sum/NR/1024)}else{print "-"}}')

put_lt1s_ssize=$(awk '$12<1000' $put_log | sort -k13 -n | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')
put_gt1s_ssize=$(awk '$12>=1000 && $12<=10000' $put_log | sort -k13 -n | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')
put_gt10s_ssize=$(awk '$12>10000' $put_log | sort -k13 -n | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')

put_lt1s_lsize=$(awk '$12<1000' $put_log | sort -k13 -nr | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')
put_gt1s_lsize=$(awk '$12>=1000 && $12<=10000' $put_log | sort -k13 -nr | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')
put_gt10s_lsize=$(awk '$12>10000' $put_log | sort -k13 -nr | awk '{print $13}' | grep -v -- "-" | head -1 | awk 'END{if (NR > 0) {printf("%.2f", $1/1024)}else{print "-"}}')
...
sort -k13 -nr $put_log | head -5 | awk '{print $1,$7,$8,$9,$10,$11,$12,$13,$14}'
echo
echo
echo "*********************** Top 5 PUT requests by Response time ***********************"
sort -k12 -nr $put_log | head -5 | awk '{print $1,$7,$8,$9,$10,$11,$12,$13,$14}'
echo
echo
fi
}
