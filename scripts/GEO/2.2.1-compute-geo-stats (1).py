...
handler = logging.handlers.RotatingFileHandler(
              '/tmp/compute-geo-stats.log', maxBytes=0, backupCount=1)
...
if len(sys.argv) > 4:
    failure_point = int(sys.argv[4])
    if len(sys.argv) > 5:
        failure_probability = float(sys.argv[5])
    else:
        failure_probability = 1.0
    log.info("will be simulating failure at point: " + str(failure_point) + ", probability: " + str(failure_probability))

progress_file = '/tmp/compute-geo-stats-progress.txt'
lock_file = "/tmp/compute-geo-stats-lock"
results_file = "/tmp/compute-geo-stats.txt"
stats_update_file = "/tmp/compute-geo-stats-update.txt"
...
def getVdcIpMap():
    try:
        ...
        for s in itemlist:
            ...
            ipList = s.getElementsByTagName("interVdcEndPoints")[0].childNodes[0].data
            vdcIpMap[vdc] = ipList
        return vdcIpMap
    except:
...
def getCtTableUrls():
    try:
        url = "http://" + thisVdcIp + ":" + dtQueryPort + "/diagnostic/CT/1/?useStyle=raw"
        ctList = []
        r = requests.get(url)
        xmldoc = minidom.parseString(r.text)
        ...
        for chunkDataType in ["REPO", "JOURNAL", "BTREE"]:
            for vdcId in vdcCosMap:
                ...
                for endPoint in endPoints:
                    ...
                    #statOutput = requests.get(queryUrl)
