...
def getVdcIpMap():
    try:
        ...
        for s in itemlist:
            ...
            if s.getElementsByTagName("managementEndPoints"):
                ipList = s.getElementsByTagName("managementEndPoints")[0].childNodes[0].data
            else:
                ipList = s.getElementsByTagName("interVdcCmdEndPoints")[0].childNodes[0].data
            vdcIpMap[vdc] = ipList
        return vdcIpMap
    except:
    ...
    for chunkDataType, statsEntry in statsGeoPending.iteritems():
        for key, value in statsEntry.iteritems():
            if (value != 0):
                ...
                f.write(updateUrl);
                f.write("\n");
    f.write("Total bytes pending replication: %d", totalPendingBytes) 
    f.close()
