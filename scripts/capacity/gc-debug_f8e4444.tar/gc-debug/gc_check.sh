...
function parse_args
{
    ...
    if [[ ${CHECK_REPO_GC} -ne 1 &&  ${CHECK_BTREE_GC} -ne 1 \
        &&  ${PRINT_TOPOLOGY} -ne 1 &&  ${CHECK_CONFIG} -ne 1 ]] ; then
...
function btree_gc_get_remaining_from_gc_query
{
    ...
    dt_query "http://${DATA_IP}:9101/gc/btreeUsage/${COS}/1" ${f_btree_gc_remaining_raw}
    ...
    awk '!/^</' ${f_btree_gc_remaining} | awk -F',' 'BEGIN{
        ...
        print "-------------------------------------------------------"
        printf("%-17s | %13.2f GB | %10.2f TB\n", "Remaining Partial  ", (garbage-full_garbage)/1073741824,(garbage-full_garbage)/1099511627776)
        print "-------------------------------------------------------"
        printf("%-17s | %13.2f GB | %10.2f TB\n", "Reclaimable Partial", reclaimable_partial_garbage/1073741824,reclaimable_partial_garbage/1099511627776)
        print "-------------------------------------------------------"
        print "  * Remaining Partial: remaining partial garbage, is a part of \"Remaining Garbage\""
        print "  * Reclaimable Partial: remaining partial garbage under GC threshold, is a part of \"Remaining Partial\""
    }'
